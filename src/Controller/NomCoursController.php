...
final class NomCoursController extends AbstractController
{
    ...
    public function index(NomCoursRepository $nomCoursRepository): Response
    {
    ...
    public function new(Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash('success', 'Le nom de cours a été créé avec succès.');
            return $this->redirectToRoute('app_nom_cours_index', [], Response::HTTP_SEE_OTHER);
        }

        return $this->render('nom_cours/new.html.twig', [
            'nom_cour' => $nomCour,
            'form' => $form->createView(), // Utilisez createView() pour le formulaire
        ]);
    }
    ...
    public function edit(Request $request, NomCours $nomCour, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash('success', 'Le nom de cours a été modifié avec succès.');
            return $this->redirectToRoute('app_nom_cours_index', [], Response::HTTP_SEE_OTHER);
        }
    ...
    {
        if ($this->isCsrfTokenValid('delete'.$nomCour->getId(), $request->request->get('_token'))) {
            $entityManager->remove($nomCour);
            $entityManager->flush();

            $this->addFlash('success', 'Le nom de cours a été supprimé avec succès.');
        }
